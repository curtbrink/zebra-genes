...
public class FirstWithChoiceConstraint(
    ...
    IReadOnlyCollection<IOrderedVariable> rangeToCheck,
    IReadOnlyCollection<int?> choiceList,
    string? expected = null,
    bool isReverse = false,
    bool isDeterminedByOwner = false)
    : BaseSelfRefConstraint<int?>(rangeToCheck.Contains(owner) ? rangeToCheck : [..rangeToCheck, owner], choiceList)
{
    // can also do:
    // - last with choice
    ...
    public override bool IsSatisfiable(IDomainStore<string> domains)
    {
        ...
        var ownerOptions = domains.GetDomain(owner).Values.ToList();
        foreach (var candidate in ownerOptions)
        {
            ...
            if (firstQId == null)
            {
                ...
                if (_window.All(q => domains.GetDomain(q).Values.Any(o => o != valueToCheck)))
                {
                    // found one
                    return true;
                }
            ...
            if (!domains.GetDomain(targetQ).Values.Contains(valueToCheck!))
            {
            ...
            {
                var qi = _window[i];
                var qiDomain = domains.GetDomain(qi).Values.ToList();
