...
public class OnlyConsecutiveSameConstraint(
    ...
    IReadOnlyCollection<List<int>> choiceList)
    : BaseSelfRefConstraint<List<int>>(scope, choiceList)
{
    public override string Name => "OnlyConsecutiveSame";

    public override string Description =>
        $"Only {_consecutiveN} consecutive same starts with {owner.Name}={{{OptionString}}}";
    ...
    public override bool IsSatisfiable(IDomainStore<string> domains)
    {
        ...
        var possibleAnswers = domains.GetDomain(owner).Values.ToList();
        foreach (var candidate in possibleAnswers)
        {
            ...
            var sliceDomains = candidateSlice.Select(c => scope.First(va => va.Id == c))
                .Select(ov => domains.GetDomain(ov).Values.ToList()).ToList();
