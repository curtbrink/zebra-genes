...
public abstract class BaseSudokuConstraint : IConstraint<int>
{
    protected List<int> SudokuValues { get; } = [1, 2, 3, 4, 5, 6, 7, 8, 9];
    ...
    public bool IsSatisfiable(IVariable? v, int val, IDictionary<IVariable, IDomain<int>> domains)
    {
        ...
        if (v is not null && SudokuValues.Contains(val))
        {
